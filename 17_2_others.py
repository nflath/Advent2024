...
with open(sys.argv[1] if len(sys.argv) > 1 else './17.in') as f:
...
# We're finding the output valiue, in reverse
while j <= len(prog) and j >= 0:
    ...
    for i in range(istart, 8):
        ...
        # No byte generated the next output; start again fro the last byte.  May have to do this multiple times.
        j -= 1
        rega >>= 3
        istart = rega%8+1
        rega >>= 3
